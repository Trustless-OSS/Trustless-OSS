...
export default async function RepoDetailPage({ params }: { params: Promise<{ repoId: string }> }) {
  ...
  return (
    <div className="w-full">
      <Link
        href="/dashboard"
        className="label-brutal text-slate-500 hover:text-slate-950 flex items-center gap-2 mb-8 transition-colors w-fit underline underline-offset-4 decoration-2"
      >
        &lt; RETURN_TO_DASHBOARD
      </Link>
      ...
      {repo && (
        <div className="bg-white brutal-border p-8 md:p-12 mb-16 brutal-shadow relative">
          ...
          <div
            ...
            className="absolute top-0 right-0 z-40 flex items-stretch"
          ></div>

          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12">
            <div>
              <div className="label-brutal text-slate-500 mb-2">TARGET_MODULE</div>
              <h1 className="title-brutal text-3xl md:text-5xl text-slate-950 mb-4">
                {repo.full_name}
              </h1>
              {repo.escrow_contract_id ? (
                ...
                <div className="flex flex-col gap-4 mt-4">
                  <span className="label-brutal text-red-600 bg-red-100 border-2 border-red-600 px-2 py-1 w-fit">
                    ERR_NO_ESCROW
                  </span>
        ...
        <div className="mb-12 p-6 bg-blue-50 border-[4px] border-slate-950 shadow-[4px_4px_0_0_#000]">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-mono text-slate-600 font-bold uppercase">
      ...
      {issues.length === 0 ? (
        <div className="bg-white brutal-border p-16 text-center brutal-shadow">
          ...
          <p className="font-mono text-sm text-slate-950">
            Append{' '}
            <span className="bg-slate-200 border-2 border-slate-950 px-1 font-black">rewarded</span>{' '}
            label with difficulty (
            <span className="bg-slate-200 border-2 border-slate-950 px-1 font-black">low</span>,{' '}
            <span className="bg-slate-200 border-2 border-slate-950 px-1 font-black">medium</span>,{' '}
            <span className="bg-slate-200 border-2 border-slate-950 px-1 font-black">high</span>),
            or comment{' '}
            <span className="bg-slate-200 border-2 border-slate-950 px-1 font-black">
              @Trustless-OSS 50
            </span>{' '}
            on target issue.
          </p>
              ...
              {issues.map(
                (
                  ...
                  return (
                    <tr
                      key={issue.id}
                      className={`text-slate-950 ${idx !== issues.length - 1 ? 'border-b-4 border-slate-950' : ''} hover:bg-slate-50 transition-colors`}
                    >
                      ...
                      <td className="px-6 py-4 border-r-4 border-slate-950">
                        ...
                        <span className="text-xs text-slate-500 font-bold">USDC</span>
                      </td>
                      <td className="px-6 py-4 border-r-4 border-slate-950">
