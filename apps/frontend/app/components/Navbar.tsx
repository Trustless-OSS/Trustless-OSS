              ...
              {breadcrumbs.map((crumb, i) => (
                <div key={crumb.label} className="flex items-center gap-4">
                  {crumb.href ? (
                    <Link
                      href={crumb.href}
                      className="text-slate-600 hover:text-blue-600 hover:underline underline-offset-4 decoration-2 transition-colors uppercase"
                    >
                      {crumb.label}
                    </Link>
                  ) : (
                    <span className="text-slate-950 uppercase bg-slate-200 px-2 py-0.5 border border-slate-950">
                      {crumb.label}
                    </span>
                  )}
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-3 bg-white brutal-border px-2 py-1 brutal-shadow">
                ...
                <form
                  action="/auth/signout"
                  method="post"
                  className="ml-2 border-l-2 border-slate-950 pl-2"
                >
                  <button
                    type="submit"
