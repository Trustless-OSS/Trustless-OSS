...
impl TrustlessOssContract {
    ...
    pub fn release_funds(env: Env, issue_id: u64) {
    ...
    pub fn partial_release(env: Env, issue_id: u64, release_amount: i128) {
        let mut escrow = storage::get_escrow(&env);
        auth::require_platform(&env, &escrow);
        auth::require_active(&env, &escrow);
        ...
        let token_client = token::Client::new(&env, &escrow.token);
        token_client.transfer(&env.current_contract_address(), &contributor, &release_amount);

        storage::set_escrow(&env, &escrow);
        storage::set_milestone(&env, issue_id, &milestone);

        let returned_to_pool = milestone.reward - release_amount;
        events::emit_partial_release(&env, issue_id, contributor, release_amount, returned_to_pool);
